...
> `ITM_SendChar()` sends characters one at a time through the **Instrumentation Trace Macrocell** — a built-in Cortex-M4 debug pathway that needs **no UART pins whatsoever**.
...
Record these values during the lab session:
