...
**Core idea:** Each task *checks* whether its scheduled interval has elapsed — none of them *waits* for it.
...
4. Resume ▶ and watch variables update live
```
...
The non-blocking super loop successfully executed three independent periodic tasks in parallel without stalling, proving that `HAL_GetTick()`-based software counters offer a lightweight and effective alternative to RTOS task scheduling for resource-constrained embedded applications.
