...
**Objective:** Connect an HC-SR04 ultrasonic sensor to the STM32F446RE, compute distance using time-of-flight measurements, and indicate range thresholds via onboard LED and UART output.
...
The HC-SR04 determines distance by measuring the **travel time** of ultrasonic pulses:
...
- **Prescaler:** `84 - 1` = 83
- **Counter Period (ARR):** leave at default (modified dynamically at runtime)
- Resulting resolution: **1 µs per tick**
...
- Enter project name → choose **STM32CubeIDE** → **Generate Code**
...
/* USER CODE BEGIN 0 */
const float speedOfSound = 0.0343 / 2;  // cm/µs, halved for round-trip
float distance;
char uartBuf[100];
/* USER CODE END 0 */
```
...
```c
/* USER CODE BEGIN 3 */
// 1. Bring TRIG low to ensure a clean start
HAL_GPIO_WritePin(TRIG_GPIO_Port, TRIG_Pin, GPIO_PIN_RESET);
usDelay(3);

// 2. Issue a 10 µs trigger pulse
HAL_GPIO_WritePin(TRIG_GPIO_Port, TRIG_Pin, GPIO_PIN_SET);
usDelay(10);
HAL_GPIO_WritePin(TRIG_GPIO_Port, TRIG_Pin, GPIO_PIN_RESET);

// 3. Wait until ECHO goes HIGH
while (HAL_GPIO_ReadPin(ECHO_GPIO_Port, ECHO_Pin) == GPIO_PIN_RESET);

// 4. Count elapsed µs while ECHO remains HIGH
numTicks = 0;
...
// 6. Transmit result via UART
sprintf(uartBuf, "Distance (cm) = %.1f\r\n", distance);
HAL_UART_Transmit(&huart2, (uint8_t *)uartBuf, strlen(uartBuf), 100);
HAL_Delay(1000);

// 7. LED proximity indicator
if (distance <= 20.0f)
...
void usDelay(uint32_t uSec) {
    if (uSec < 2) uSec = 2;
    usTIM->ARR = uSec - 1;   // Load the desired count
    usTIM->EGR = 1;           // Reinitialize counter and prescaler
    usTIM->SR  &= ~1;         // Clear the update event flag
    usTIM->CR1 |= 1;          // Enable the counter
    while ((usTIM->SR & 0x0001) != 1); // Spin until overflow
    usTIM->SR &= ~(0x0001);
}
/* USER CODE END 4 */
```

> **Why use direct register access?** `HAL_Delay()` operates only at 1 ms resolution — far too coarse for the microsecond-level timing this sensor demands.
...
1. Why is TIM4's prescaler configured as `84 - 1`?
2. How do registers `ARR`, `EGR`, `SR`, and `CR1` collaborate inside `usDelay()`?
3. What is the purpose of calling `usDelay(2)` within the echo pulse measurement loop?
4. What does the guard condition `if(uSec < 2) uSec = 2` protect against?
...
The HC-SR04 ultrasonic sensor was successfully interfaced with the STM32F446RE. Distance values were derived using `d = (v × Δt) / 2` and streamed to a terminal via UART. The onboard LED illuminated whenever the measured distance dropped below the 20 cm threshold, confirming correct sensor integration and proximity detection.
