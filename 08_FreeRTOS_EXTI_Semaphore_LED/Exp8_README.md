...
Configure an **EXTI interrupt** on the onboard user button (PC13) and employ a **Binary Semaphore** to synchronize an LED control task — implementing the industry-standard **Deferred Interrupt Processing** pattern.
...
>  FreeRTOS mandates that any ISR calling FreeRTOS API functions must have its NVIC priority set **≥ configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY** (typically numerically 5 or higher). Priority 7 satisfies this.
...
| Initial Count | `1` *(will be manually overridden to 0 in code — see below)* |
...
void StartDefaultTask(void *argument) {
    uint8_t i;
    for (;;) {
        /* Wait here indefinitely until ISR releases the semaphore */
...
```c
//  Auto-generated (wrong — task runs immediately at startup without a press):
...
> ** This manual change must be reapplied each time CubeMX code is regenerated.**
...
1. Why doesn't `LED_Control` continue running indefinitely after the button is pressed?
2. What is a breakpoint and how can it be leveraged in debug mode for this experiment?
