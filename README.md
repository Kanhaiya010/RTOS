...
This repository documents a series of hands-on experiments with the **STM32F446RE Nucleo Board**, covering everything from foundational bare-metal GPIO programming to real-time operating system implementation using **FreeRTOS**, **STM32CubeIDE**, and **STM32CubeMX**.

Every folder in this repository represents a self-contained project targeting a specific peripheral, architectural concept, or RTOS feature of the STM32 microcontroller platform.
...
- PWM Signal Generation and Duty Cycle Manipulation
- Non-Blocking Super Loop Design with Software Timing Counters
- FreeRTOS Task Creation and Scheduler Basics
...
|--------|------------|-------------|
| `01_GPIO_LED_Blink_SoftwareDelay` | GPIO Digital Output — LED Blink | Configure a GPIO pin as a push-pull digital output and verify LED blinking behavior using software delay routines |
| `02_PushButton_LED_Toggle` | Digital Input — Push Button LED Toggle | Interface the onboard push button as a digital input and toggle the LED state on every valid button press |
| `03_HCSR04_Ultrasonic_LED_RangeIndicator` | HC-SR04 Ultrasonic Sensor — Distance Classification | Interface an HC-SR04 ultrasonic sensor with the STM32F446RE and indicate distance ranges using LED visual feedback over UART |
...
- Experiments 1 through 5 follow a **bare-metal, HAL-based** approach centered on polling loops and software delays.
...
- Every project folder contains the complete STM32CubeIDE project along with source code and the CubeMX configuration file.
