...
> A counting semaphore is a **gated pool with multiple simultaneous entries**.

---

##  Objective

Model a **capacity-limited shared resource** using a FreeRTOS Counting Semaphore shared across three competing tasks, and study resource contention, concurrent access control, and task interleaving through the SWV ITM Data Console.
...
│ Counting Semaphore│ N (settable)│ Resource pools│ No (N at a time)  │
└──────────────────┴──────────────┴───────────────┴────────────────────┘
```
...
With semaphore count = **2**, tasks A and B hold it simultaneously → both write to ITM at the same time → **garbled output**:

```
Expected (if using a mutex):     Actual (counting semaphore, count=2):
  1AAAAAAAAAA                      1A2BAB2BBBAAAB1AAAB2BB
  2BBBBBBBBBB                      (characters interleaved — unpredictable)
  3CCCCCCCCCC
```

This is **intentional, not a defect** — it perfectly demonstrates that counting semaphores manage *how many* tasks may concurrently access a resource, but not *the order or atomicity* of those accesses.
...
void func_TaskA(void *argument) {
    ...
    for (;;) {
        ...
        printf("1");
        for (int i = 0; i < 10; i++) {
            printf("%c", ch);   // Outputs 'A' ten times
            HAL_Delay(50);
        }
...
void func_TaskB(void *argument) {
    ...
    for (;;) {
        ...
        printf("2");
        for (int i = 0; i < 10; i++) {
            printf("%c", ch);   // Outputs 'B' ten times
            HAL_Delay(50);
        }
...
void func_TaskC(void *argument) {
    ...
    for (;;) {
        ...
        printf("3");
        for (int i = 0; i < 10; i++) {
            printf("%c", ch);   // Outputs 'C' ten times
            HAL_Delay(50);
        }
        osSemaphoreRelease(myCountingSem01Handle);
        osDelay(5);
...
With count=2, two tasks hold the semaphore concurrently:
   ...
   (interleaved — two tasks printing simultaneously)
...
| 2 | Describe the character interleaving pattern in the trace output | |
| 3 | Type of scheduling responsible for the interleaving behavior | |
...
| Omit `osSemaphoreRelease()` from a task | That task permanently holds the token — others may starve |
| Raise one task to `osPriorityHigh` | That task wins the token more frequently |
| Swap counting semaphore for a **mutex** | True mutual exclusion — clean sequential output, one task at a time |
...
Counting Semaphore (count=2):
     Controls HOW MANY tasks may enter a shared region concurrently
     Does NOT enforce atomic or exclusive access for each individual task

Mutex:
     Guarantees exactly ONE task at a time (true mutual exclusion)
     Carries ownership — only the acquirer may release it
     Not intended for task-to-task signaling
...
1. What does "thread-safe" mean — and why does a counting semaphore with count > 1 not fully guarantee it for a single ITM/UART port?
...
4. Write a summary of your key learnings about RTOS-based system design accumulated across all experiments in this series.

---

##  Result

Three tasks competed for a shared resource (the ITM trace port) governed by a Counting Semaphore initialized to count = 2. The SWV output clearly revealed task interleaving — two tasks wrote simultaneously, producing irregular mixed character sequences. This demonstrated that counting semaphores control resource **availability** rather than **atomicity**, drawing a sharp distinction from mutexes in practical RTOS system design.
