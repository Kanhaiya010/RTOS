...
Set up two FreeRTOS tasks with adjustable priority levels and analyze how the **preemptive scheduler** distributes CPU time between them — verified through LED blink behavior and SWV ITM trace console output.
...
Modify priorities in the CubeMX Tasks & Queues tab, regenerate code, then observe behavior:
...
2. What happens to the LEDs when one task has no `osDelay()` call at all?
3. Compare this dual-task RTOS implementation with a super loop controlling two LEDs — what structural advantages does the RTOS offer?
...
Two FreeRTOS tasks were created with programmable priorities using the CMSIS-RTOS v2 interface. Priority-based preemptive scheduling behavior was clearly observed through differential LED blink rates and SWV ITM console output, illustrating how a higher-priority task monopolizes CPU access and can lead to starvation of lower-priority tasks.
